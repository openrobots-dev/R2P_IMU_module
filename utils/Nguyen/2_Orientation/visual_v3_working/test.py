...
import OrientationManager as QuaternionManager #MYCODE or QuaternionReadSerialPort
from time import sleep #MYCODE
...
ser = QuaternionManager #MYCODE    #serial.Serial(port='COM9',baudrate=115200, timeout=1)
...
label(pos=(-0.9,0.3,0),text="Clock",box=0,opacity=0)
...
L0 = label(pos=(-0.9,0.22,0),text="-",box=0,opacity=0)
...
while 1:
    ...
    #    if len(words) > 2:
    #        try:
    #            roll = float(words[0]) #*grad2rad   #MYCODE ignored grad2rad, because it is calculated in quaternion method
    #            pitch = float(words[1]) #*grad2rad  #MYCODE ignored grad2rad, because it is calculated in quaternion method
    #            yaw = float(words[2]) #*grad2rad    #MYCODE ignored grad2rad, because it is calculated in quaternion method
    #        except:
    #            print "Invalid line"

    roll = float(line[0]) #*grad2rad   #MYCODE ignored grad2rad, because it is calculated in quaternion method
    pitch = float(line[1]) #*grad2rad  #MYCODE ignored grad2rad, because it is calculated in quaternion method
    yaw = float(line[2]) #*grad2rad
    ...
    L0.text = str(count)
    L1.text = str(float(line[0]))
    L2.text = str(float(line[1]))
    L3.text = str(float(line[2]))
    sleep(0.000000000000000000000000001) #MYCODE to refresh drawing
ser.close
#f.close
