#-------------------------------------------------------------------------------
# Name:        IMU - visualize orientation
# Purpose:
...
#
# Author:      Nguyen Ho Thi Thao (mail: nguyen.ho@mail.polimi.it)
#
# Created:     26/02/2013
# Copyright:   (c) nguyen 2013
# Licence:     <your licence>
#-------------------------------------------------------------------------------
import OrientationManager as OrientationManager
...
# Main scene
scene=display(title="Polimi - IMU AHRS")
...
scene2 = display(title='Polimi - IMU AHRS',x=0, y=0, width=500, height=200, center=(0,0,0), background=(0,0,0))
scene2.range=(1,1,1)
scene.width=500
scene.y=200
scene2.select()
...
label(pos=(0.6,0.16,0),text="N",box=0,opacity=0,color=color.yellow)
...
label(pos=(0.75,0.13,0),height=7,text="NE",box=0,color=color.yellow)
label(pos=(0.45,0.13,0),height=7,text="NW",box=0,color=color.yellow)
...
L3 = label(pos=(0.7,0.22,0),text="-",box=0,opacity=0)
count = 0
...
# labels
label(pos=(0,0,0.8),text="Polimi - IMU AHRS",box=0,opacity=0)
...
#initialize step (open port, open file...)
...
while 1:
    line = orientationManager.readline()

    if(line != None):
        roll = float(line[0]) #*grad2rad
        pitch = float(line[1]) #*grad2rad
        ...
        sleep(0.00000001) #refresh drawing
