#-------------------------------------------------------------------------------
# Name:        device orientation - visualize the display
# Purpose:
#
# Author:      nguyen
#
...
from OrientationManager import OrientationManager
...
while 1:
    line = ser.readline()
    line = line.split()
    #print (int(line[1]),int(line[2]),int(line[3]),int(line[4]),int(line[5]),int(line[6]),int(line[7]),int(line[8]),int(line[9]))
    (gx, gy, gz, ax, ay, az, mx, my, mz) = calManager.calibrate_raw_measures(int(line[1]),int(line[2]),int(line[3]),int(line[4]),int(line[5]),int(line[6]),int(line[7]),int(line[8]),int(line[9]))
    #print (gx, gy, gz, ax, ay, az, mx, my, mz)

    q = calManager.quaternionUpdate(gx, gy, gz, ax, ay, az, mx, my, mz)
    #print "euler:"
    (phi, theta, psi) = calManager.quaternionToEuler(q[0],q[1],q[2],q[3])
    print (phi, theta, psi)
'''
    roll = phi
    pitch = theta
