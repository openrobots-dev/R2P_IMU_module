...
sleep(1)
serial_port.write("smad " + str(frequency) + "\r\n")
sleep(1)
serial_port.flushInput()
...
while (count < samples) or (samples < 0) :
	...
	if len(words) > 2:
		...
		L1.text = str(round(float(words[1]) * rad2grad, 2))
		L2.text = str(round(float(words[2]) * rad2grad, 2))
		L3.text = str(round(float(words[3]) * rad2grad, 2))
